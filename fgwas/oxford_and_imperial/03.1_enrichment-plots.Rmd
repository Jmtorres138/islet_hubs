...
fgwas.dir <- serv.dir %&% "projects/islet_hubs/fgwas/oxford_and_imperial/"
...
# Additional re-naming desired 

```{r}

plotfile <- serv.dir %&% "projects/islet_hubs/Thurner-name-key.txt"
k.df <- fread(plotfile)
...
param.sing.df$parameter <- map(param.sing.df$parameter,function(annot){
  val <- filter(k.df,ShortName==annot)$FullName
  ifelse(length(val)==0,annot,val)
}) %>% as.character(.)
rem.vec <- c("promoter","exon","intron","transcript","utr_3","utr_5")
param.sing.df <- filter(param.sing.df,!(parameter %in% rem.vec))
...
param.df$parameter <- map(param.df$parameter,function(annot){
  val <- filter(k.df,ShortName==annot)$FullName
  ifelse(length(val)==0,annot,val)
}) %>% as.character(.)
rem.vec <- c("promoter","exon","intron","transcript","utr_3","utr_5")
param.df <- filter(param.df,!(parameter %in% rem.vec))
